...
from scipy.signal import argrelextrema # get local maxima in range 0 : 600
maxima = argrelextrema(np.array(kCnt[20:1800]), np.greater)
mx = []
my = []
for maximum in maxima[0] :
    if kCnt[maximum+20] > 195000 :
...
for x, y in zip(mx, my) :
    ...
    if len(globx) == 0 :
        ...
        globx.append(x)
        globy.append(y)
    elif globx[-1] != x and globx[-1]*1.1 >= x and globy[-1] < y : 
        globx[-1] = x
        globy[-1] = y
...
gnsz = sum([x*y for x,y in zip(kFrq[40:], kCnt[40:])])/183

ax.annotate(xy=(-10, -0.04*max(kCnt[20:1000])), s="Estimated genome size: " + str(round(gnsz, 2))+"bp")
plt.show()

print("Estimated Genome Size: " + str(round(gnsz, 2)) + " (total sum divided by 183)")
